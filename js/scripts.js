...
function buttonMaker() {
    ...
    for (let i = 0; i < str.length; i++) {   // Create hidden letter places
        counter++;
        let p = `<input type="text" id="letter${counter}" maxlength="1" onkeyup="check(${counter})" name="${counter}" value="" style="font-size: 28px;" autocapitalize="none"/>`; // Create letter places  onkeyup
        buff += `${p} `
    }
